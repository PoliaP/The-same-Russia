    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            with transaction.atomic():
                ...
                for i, temp_id in enumerate(new_files_order_temp_ids):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            with transaction.atomic():
                ...
                max_order = NewsImage.objects.filter(news=news_item).aggregate(models.Max('order'))['order__max']
...
def history_list(request):
    lines = HistoryLine.objects.all().order_by('-id')
...
def home(request):
    ...
    history_lines = HistoryLine.objects.all().order_by('-id')[:3]
    partners = Partner.objects.all()
